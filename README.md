...
| `_elevate_client_build_default_ctx(ctx_buf_out, explicit_timeout_ns) -> () !{mem} @{}` | LE.M1-002 (private-by-convention): shared "hidden default-context helper" used by the three legacy-wrapper entry points (`_ex_j`, `_ex_r`, `_acquire`). Snapshots the three process-global mutables into a caller-owned 32-byte ctx buffer (fast / human default sentinels stay 0; the ctx variant's own timeout resolution substitutes the constants). |
| `elevate_client_cap_drain_broker_exp(reply_ep_id) -> u64 !{mem} @{cap}` | LE.M7-002 (#36): non-blocking single-sweep drain of `reply_ep_id` for pending `ELV_OP_EXP` frames, forwarding each to `elevate_client_cap_revoke_cascade` by row_id and summing the returns. Kernel does not emit `ELV_OP_EXP` today (client-side scaffolding; see the file header). Reuses `ELVC_ERR_BAD_REPLY_EP` for the `[1..127]` gate. |
...
| Signature | Purpose |
| --- | --- |
| `elevate_client_acquire(caps, dur, req_buf, reply_ep_id, reply_buf, mint_ctx_buf) -> u64 !{mem} @{boot, cap}` | Full `request_ex_r` flow, then mint + shadow-bind (expire + granted caps). Returns `row_id` (`< 16`) on success; every failure passes through the ORIGINAL band from whichever layer refused, or `ELCA_ERR_BAD_BUF` if `mint_ctx_buf == 0`. `mint_ctx_buf` is a 40-byte, 5-word caller buffer: `parent_ep_slot`, `request_id`, `requester_pid`, `target_cap_kind`, `target_cap_rights` (see the file header for the exact layout). LE.M1-002 refactor: this is now a thin wrapper over `elevate_client_acquire_ctx` via `_elevate_client_build_default_ctx` (globals snapshot → 7-arg delegation); public 6-arg signature and byte-identical behaviour preserved. |
| `elevate_client_acquire_ctx(caps, dur, req_buf, reply_ep_id, reply_buf, mint_ctx_buf, ctx_buf) -> u64 !{mem} @{boot, cap}` | LE.M1-002 (#20): explicit-context twin of `elevate_client_acquire`. SEVEN-argument SysV call (7th arg on stack) — the first in this library to use paideia-as's 7-arg support. Dispatches step 2 through `elevate_client_request_ex_r_ctx` so `ctx_buf` (32-byte, ENH-006 layout) carries target_fp_lo/fast/human/explicit_timeout end-to-end; `mint_ctx_buf` layout is unchanged. Gate priority: `ctx_buf==0` (widest surface) → `ELVC_ERR_BAD_CTX`; then `mint_ctx_buf==0` → `ELCA_ERR_BAD_BUF`; then `reply_buf==0` → `ELCA_ERR_BAD_REPLY_BUF`. |
...
| `elevate_client_cap_scrub_row(row_id) -> () !{mem} @{}` | LE.M7-001 (#35): zero all eight per-row shadow maps for `row_id`. Factored out of `_check_and_revoke` / `_derive`'s own inline scrubs as the third call site (`revoke_cascade`, below). Out-of-range `row_id` is a silent no-op. |
| `elevate_client_cap_revoke_cascade(row_id) -> u64 !{mem} @{cap}` | LE.M7-001 (#35): force-revokes `row_id` and every transitive parent-map descendant (LE.M3-002), deepest generation first. Returns the revoked count (self + descendants); a kernel refusal other than "already revoked" aborts immediately with `ELCC_ERR_CASCADE_FAIL = 0xFFFFEA74`. Idempotent — a second call on an already-swept chain returns `0`. Best-effort journals each revoke via `elevate_client_journal_revoke_cascade`. |
...
| `elevate_client_request_ex_j(caps, dur, req_buf, reply_ep_id, reply_buf, timeout_ns) -> u64 !{mem} @{boot}` | Audit-wrapped flow: journal REQ (failure aborts *before* the wire hop) → `request_ex` → journal APR. An APR-journal failure after a grant surfaces so the caller can revoke. LE.M1-002 refactor: now a thin wrapper over `elevate_client_request_ex_j_ctx` via `_elevate_client_build_default_ctx`; public signature and byte-identical behaviour preserved. |
| `elevate_client_request_ex_j_ctx(caps, dur, req_buf, reply_ep_id, reply_buf, ctx_buf) -> u64 !{mem} @{boot}` | LE.M1-002 (#20): explicit-context twin of `_ex_j`. Reads `target_fp_lo` from `ctx_buf[+0]` and dispatches through `elevate_client_request_ex_ctx` so the same ctx (32-byte, ENH-006 layout) carries target_fp_lo / timeouts end-to-end -- one shell brokering N concurrent jobs races nothing. `ELVC_ERR_BAD_CTX` if `ctx_buf == 0`. |
...
| `elevate_client_journal_revoke_cascade(actor_fp_lo, row_id, cascade_root_row_id) -> u64 !{mem} @{}` | LE.M7-001 (#35): append an `ELVJ_EVT_REVOKE_CASCADE = 5` record per row `elevate_client_cap_revoke_cascade` revokes (`body1 = row_id` just revoked, `body2 = cascade_root_row_id`). Returns the seq or `ELVJ_ERR_BAD_ACTOR` / `_REVOKE_CASCADE_JOURNAL_FAIL`; the caller treats this as best-effort. |
...
| `elevate_client_request_ex_r(caps, dur, req_buf, reply_ep_id, reply_buf, timeout_ns) -> u64 !{mem} @{boot}` | Retry-wrapped full flow. Retriable set is exactly `{ELVC_ERR_TIMEOUT, ELVC_ERR_SEND_FAIL, ELVC_ERR_LOOKUP_FAIL}`; everything else passes through. Exhaustion returns `ELVR_ERR_EXHAUSTED`. LE.M1-002 refactor: now a thin wrapper over `elevate_client_request_ex_r_ctx` via `_elevate_client_build_default_ctx`; public signature and byte-identical behaviour preserved. |
| `elevate_client_request_ex_r_ctx(caps, dur, req_buf, reply_ep_id, reply_buf, ctx_buf) -> u64 !{mem} @{boot}` | LE.M1-002 (#20): explicit-context twin of `_ex_r`. Each retry attempt fires through `elevate_client_request_ex_j_ctx` so actor identity + timeout classification stay stable across retries (a per-attempt globals read cannot race the retry loop). `ELVC_ERR_BAD_CTX` if `ctx_buf == 0`; the bad-arg return does NOT charge a retry attempt (`ELVR_ST_ATTEMPTS` unchanged). |
...
(journal — now 6 members with `ELVJ_ERR_ATTEST_JOURNAL_FAIL`, LE.M5-001
#31, and `ELVJ_ERR_REVOKE_CASCADE_JOURNAL_FAIL`, LE.M7-001 #35),
`ELCA_*` `0xFFFFEA50..5F` (acquire, ENH-001), `ELCA_ERR_*`
...
and an `ELCC_*` extension band `0xFFFFEA70..7F` (LE.M3 + LE.M7-001:
`ERR_CYCLE_DETECTED`, `ERR_BAD_CTX`, `ERR_ZERO_GRANT`, `ERR_CASCADE_FAIL`
— see the "EXTENSION BAND" header in `src/elevate_client_cap.pdx` for
why these four sit outside the base `0x30..3F` band). `0` is success
in every band. The boot witness
